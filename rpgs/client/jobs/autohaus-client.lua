...
local musicares = playSound3D ( "http://www.top100station.de/switch/top100station_lq.asx", 1087.4, 1751.8, 10.8, true )
setSoundMaxDistance ( musicares, 30 )

local musicc4y = playSound3D ( "http://listen.technobase.fm/dsl.asx", 926, -1180.8, 17, true )
setSoundMaxDistance ( musicc4y, 25 )

local musicpac = playSound3D ( "http://listen.technobase.fm/dsl.asx", 285.6, -1171.4, 80.9, true )
setSoundMaxDistance ( musicpac, 30 )
        ...
        if button == "left" then
			...
			if getElementData(getPedOccupiedVehicle(getLocalPlayer()), "preis") > getPlayerMoneyEx ( getLocalPlayer() ) then
				...
				if getElementData(getPedOccupiedVehicle(getLocalPlayer()), "isWangVehicle") == true then
					...
					triggerServerEvent("addJobMoney", getRootElement(), "InterTrans", math.floor(getElementData(getPedOccupiedVehicle(getLocalPlayer()), "preis")/3))
					triggerServerEvent("addJobMoney", getRootElement(), "AutoScout45", math.floor(getElementData(getPedOccupiedVehicle(getLocalPlayer()), "preis")/3))
...
function buyKaufGui2(button)
        if button == "left" then
			...
			if getElementData(getPedOccupiedVehicle(getLocalPlayer()), "preis") > getPlayerMoneyEx ( getLocalPlayer() ) then
				...
				setElementData ( getPedOccupiedVehicle(getLocalPlayer()), "reserviert", "Niemand")
				if owner == "5" then
					triggerServerEvent("addJobMoney", getRootElement(), "AutoScout45", getElementData(getPedOccupiedVehicle(getLocalPlayer()), "preis"))
