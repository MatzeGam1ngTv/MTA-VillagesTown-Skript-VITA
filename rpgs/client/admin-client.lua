...
function showHideTheUserPanelMenu()
	if guiGetVisible(AdminUserPanel_Window[1]) == false then
		...
		showTutorialMessage("admin_1", "Herzlich Willkommen bei uns im 'Villages Town' Team. Wir freuen uns auf die gemeinsame Zusammenarbeit! :-)")
		guiGridListClear(AdminUserPanel_Grid[1])
				...
				if theselecteditem ~= -1 then
					...
					if string.len(selectedjobname) ~= 0 then
						...
						if selectedjobname == "Arbeitslos" then
							jobid = 0
						elseif selectedjobname == "Ares Autohaus" then
							jobid = 1
						elseif selectedjobname == "InterTrans" then
							jobid = 2
						elseif selectedjobname == "Polizist" then
							jobid = 3
						elseif selectedjobname == "Autofix" then
							jobid = 4
						elseif selectedjobname == "AutoScout45" then
							jobid = 5
						elseif selectedjobname == "Beamter" then
							jobid = 6
						elseif selectedjobname == "Krankenhaus" then
							jobid = 7					
						elseif selectedjobname == "Feuerwehr" then
							jobid = 8		
						elseif selectedjobname == "San News" then
							jobid = 9
						end						
						local key = "job"
			...
			if source == AdminPlayerRang_Button[2] then -- OK Button
				...
				if theselecteditem ~= -1 then
					...
					if string.len(selectedrangname) ~= 0 then
						local rangid = 0
						if selectedrangname == "Azubi" then
							rangid = 0
						elseif selectedrangname == "Angestellter" then
							rangid = 1
						elseif selectedrangname == "Vorgesetzter" then
							rangid = 2
						end
						local key = "rank"
				...
				if theselecteditem ~= -1 then
					...
					if string.len(selectedrightname) ~= 0 then
						...
						elseif selectedrightname == "Administrator" then
							rightid = 3
						elseif selectedrightname == "Entwickler" then
							rightid = 4
						elseif selectedrightname == "Projektleiter" then
							rightid = 5
						end
						local key = "admin"
