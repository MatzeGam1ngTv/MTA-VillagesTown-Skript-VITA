...
function getTrailerPos()
	...
	if trailerVeh <= 20 then
		...
		print("Villages Town Online: Hafen voll!")
		return 0.0, 0.0, 0.0;
	end
end
	...
	if getElementData(ply, "job") == 2 and getElementData(ply, "dienst") == 1 then -- LKW-Fahrer
		...
		if trailer then
			if getElementData( trailer, "eingepackt" ) == true then
				...
				systemTransfer("Autohaus", "Trucker", 1250)
				setElementData(ply, "einnahmen", getElementData(ply, "einnahmen")+1250)
				setElementData(ply, "einnahmen2", getElementData(ply, "einnahmen2")+1250)
...
function truckerwho(source, commandName)
	if getElementData(source, "job") == 2 then
		...
		for i,v in ipairs(adac_repair) do
			if getElementData(v, "repairs") < 10 then
