...
function loadAllRootRPGVehicles()
	setTimer(benzinVerbrauch, 30000*5, 0)
	...
	if result then
		...
		while true do
			...
			if isFake == "true" then
				...
				if ( isElement(thevehicle) ) then
					...
					if keys == "Autohaus" or keys == "AutoScout45" or keys == "Car4you" and eingepackt ~= "true"  then
						setElementFrozen(thevehicle, true)
						setVehicleDamageProof(thevehicle, true)
					end				
					if panne == "true" then
		...
		print("Villages Town Online: Fahrzeuge wurden nicht geladen!")
	end	
...
function enterVehicle ( vehicle, seat, jacked )
	...
	if (keys == "Car4you" and getElementData(vehicle, "onFire") ~= true and getElementData(source,"job") == 11 and seat == 0) or (keys == "Autohaus" and getElementData(vehicle, "onFire") ~= true and getElementData(source,"job") == 1 and seat == 0) or (keys == "AutoScout45" and getElementData(vehicle, "onFire") ~= true and (getElementData(source,"job") == 5 or getElementData(source,"job") == 11) and seat == 0)  then
		setElementFrozen(vehicle, false)
	elseif getElementData(vehicle, "onFire") ~= true and isVehicleLocked(vehicle) ~= true and getElementData(vehicle, "blockUnfreeze") ~= true and getElementData(vehicle, "marked") ~= true and keys ~= "Autohaus" and keys ~= "AutoScout45" and keys ~= "Car4you" then
		setElementFrozen(vehicle, false)
	end
...
function exitVehicle ( vehicle, seat, jacked )
	...
	if (keys == "Autohaus" or keys == "Car4you" or keys == "AutoScout45") and seat == 0 then
...
local verwerterMarker = createMarker ( 2741.0, -2009.5, 12.1, "cylinder", 4, 150, 150, 150, 150 )
...
function verwerterMarkerFunc( hitElement, matchingDimension )
	if getElementType(hitElement) == "player" then
		...
		if veh then
			local vero = getElementData(veh, "preis")*0.40
			outputChatBox( "Hier ist Üzgür: Du willst das Auto nicht mehr?, Dann mach /autoverwertung in deinen Chat.", hitElement, 255, 255, 0 )
			outputChatBox( "Du bekommst für dein Auto.. "..tostring(vero).." Vero!", hitElement, 255, 255, 0 )
		else
			outputChatBox( "Hier ist Üzgür: Komm mit deinen Autos die du nicht mehr willst!", hitElement, 255, 255, 0 )
		end
	end
end
			...
			if getElementData(veh, "isVehicleTemp") == false then
				if (getElementData(veh, "owner") == getPlayerName(player)) or (getElementData(veh, "owner") == tostring(getElementData(player, "job")) and getElementData(player, "rank") == 2) or (getElementData(veh, "owner") == "g"..getElementData(player, "gang") and getElementData(player, "gangrank") == 2)  then
					if getElementData(veh, "owner") == "11" or getElementData(veh, "owner") == "5" or getElementData(veh, "owner") == "1" then
						return outputChatBox( "Ey! Das Fahrzeug ist nicht zuverkauften und es gehört ein Fahrzeughändler. Das nehme ich nicht! ", player, 255, 0, 0 )
					end
					local vero = getElementData(veh, "preis")*0.40
					givePlayerMoneyEx(player, vero)
					systemDeposit("AutoScout45", math.floor(vero/2))
					systemDeposit("Car4you", math.floor(vero/2))
					outputChatBox( "Das Auto wurde in das Flugzeug Verladen und wirt weiter Verkauft!", player, 0, 255, 0 )
					outputChatBox( "Du bekommscht für dein Auto.. "..tostring(vero).." Vero!", player, 0, 255, 0 )
					destroyElement(veh)
				else
					outputChatBox( "Dieses Fahrzeug hat keine Papiere, Somit Kaufe ich das Fahrzeug nicht!", player, 255, 0, 0 )
				end
			end
		end
	end
