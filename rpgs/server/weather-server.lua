	...
	if isOnline == false then
		for i,v in ipairs(getElementsByType("player")) do
			if getElementData(v, "viNetworkMuted") ~= true then
				outputChatBox("[Radio] Laura die Wetterfee: Herzlich Willkommen zur Wettervorhersage.", v, 173,255,47,true)
				outputChatBox("[Radio] Laura die Wetterfee: Das nächste Wetter wird '"..weather.sname.."'.", v, 173,255,47,true)
			end
		end
	end
end
...
addEventHandler( "onResourceStart", getResourceRootElement(getThisResource()),
	function( )
		updateWeather( )
		setTimer( updateWeather, 20 * 300000, 0 )
	end
)