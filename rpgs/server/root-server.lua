...
function onRPGResourceStart()
	...
	--Rest
	createBlip ( 1457.4, -1137.5, 10, 45, 2, 255, 0, 0, 255, 0, 100, getRootElement() )
	ped = createInvinciblePed ( 141, 161.3, -81, 1001.8, 180 )
	setElementInterior(ped, 18)
	setElementDimension(ped, 0)
	setElementData(ped, "isSkinPed", true)
	setElementData(ped, "PedName", "Leonie")
	...
	setElementData(ped, "PedName", "Fred")
	...
	setElementData(ped, "isDonutPed", true)
	setElementData(ped, "PedName", "Ronny")	
	
	ped = createInvinciblePed (168,380.883,-187.562,1000.632,90)
	setElementInterior(ped, 17)
	setElementDimension(ped, 3)
	setElementData(ped, "isDonutPed", true)
	setElementData(ped, "PedName", "Björn")		
	...
	setElementData(ped, "PedName", "Bertha")
	...
	setElementInterior(ped, 3)
	setElementData(ped, "PedName", "Security")
	...
	setElementData(ped, "PedName", "Borin")
	setElementData(ped, "cinfo", {"Einkaufen"})	
...
function checkRealTime()
	...
	--GamemodeRestart
	if second == 0 and hours == 23 and minutes > 55 then
		outputChatBox ( "#FF0000Automatischer Gamemode-Restart um #00FF0000:00", getRootElement(), 255, 255, 255, true )
	end
	if second == 0 and hours == 0 and minutes == 0 then
	...
	if isElement(player) and getElementData(player, "isPlayerLoggedIn") == false then
		setElementData(player, "isPlayerLoggedIn", "logginin")
				...
				if query and mysql_num_rows(query) > 0 then	
					...
					print("Village Town Online: "..getPlayerName(player).." joined the game ( Skin:"..tonumber(skin).." )")
...
function output()
	...
	outputChatBox ( "Willkommen auf Villages Town", source, 255, 0, 0 )	
...
function setBoughtSkin(gender, skin, abort)
	if abort ~= true then
		if getPlayerMoneyEx(source) >= 80000 then
			...
			triggerClientEvent ( source, "addNotification", getRootElement(),  1, 0, 255, 0, "Erfolgreich!\nWillkommen in deinem neuen Körper." )
		else
	...
	setElementPosition(source, 161.4, -88.2, 1001.8 )
